...
are given original names combining the module identity (\MOD{q}{\subst{A}{\hv{A}}}{X})
...
It is important that export matching happen as a complete pass before
...
Second, we form a graph representation of both the syntactic merge,
as well as the original signatures.  However, original names
are always resolved to the merged interface.  For example, in the
second row of the diagram above, there are three definitions of
...
Finally, we check that each of the original declarations from the signatures
is compatible with the merged declaration, under the graph representation
we computed previously.  Because all original names have been resolved to
