...
\begin{enumerate}
    ...
    when performing type class resolution.  (Non-orphan instances are
    always assumed to be in scope.)
...
to lookup the declaration corresponding to an original name.  In plain
Haskell, the declarations for each original name are recorded directly
in the context; in \Backpack{}, we may need to \emph{substitute} the
declaration, substituting all name holes.  \textsc{TUnit} contains the
key rule: to determine the type of a module $\Mod{p[S]}{m}$, lookup the
type and apply its substitution.
...
The separate signature type lookup judgment is required to accommodate
the case when we want to look up the type of a signature for signature
merging: at this point in time, we haven't added any type for this
signature to the local context $\Delta$, so ordinary module type lookup
will fail for this signature.  \textsc{SNameHoleSig} handles the
special case, where if a name hole corresponds to the signature we are
merging (the $m$ parameter to \textsf{substsig}), we just directly
rename it without consulting the context.  Otherwise, signature substitution
proceeds exactly like regular substitution. (\textsc{SNameSig}).
